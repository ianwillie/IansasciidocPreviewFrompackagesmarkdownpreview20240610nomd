...
const IansasciidocPreviewfrompackagesmarkdownpreview20240610nomdView = require('../lib/iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd-view')
const TextMateLanguageMode = new TextEditor().getBuffer().getLanguageMode()
  .constructor

describe('IansasciidocPreviewfrompackagesmarkdownpreview20240610nomdView', function () {
  let preview = null
  ...
  beforeEach(function () {
    ...
    preview = new IansasciidocPreviewfrompackagesmarkdownpreview20240610nomdView({ filePath })
    ...
    waitsForPromise(() => atom.packages.activatePackage('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd'))
  })
  ...
  describe('serialization', function () {
    ...
    it('does not recreate a preview when the file no longer exists', function () {
      const filePath = path.join(temp.mkdirSync('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd-'), 'foo.md')
    ...
    it('serializes the editor id when opened for an editor', function () {
      ...
      runs(function () {
        preview = new IansasciidocPreviewfrompackagesmarkdownpreview20240610nomdView({
  ...
  describe('image resolving', function () {
    ...
    describe('when the image uses an absolute path that exists', function () {
      it("doesn't change the URL when allowUnsafeProtocols is true", function () {
        ...
        atom.config.set('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.allowUnsafeProtocols', true)
    ...
    it('removes the URL when allowUnsafeProtocols is false', function () {
      ...
      atom.config.set('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.allowUnsafeProtocols', false)
  ...
  describe('gfm newlines', function () {
    describe('when gfm newlines are not enabled', function () {
      it('creates a single paragraph with <br>', function () {
        atom.config.set('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.breakOnSingleNewline', false)
    ...
    describe('when gfm newlines are enabled', function () {
      it('creates a single paragraph with no <br>', function () {
        atom.config.set('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.breakOnSingleNewline', true)
  ...
  describe('when core:save-as is triggered', function () {
    ...
    it('saves the rendered HTML and opens it', function () {
      ...
      const markdownPreviewStyles = [
        {
          rules: [createRule('.iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd', '{ color: orange; }')]
        },
        {
          rules: [
            createRule('.not-included', '{ color: green; }'),
            createRule('.iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd :host', '{ color: purple; }')
          ]
        }
      ]
      ...
      const atomTextEditorStyles = [
        ...
        'atom-text-editor .hr { background: url(atom://iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd/assets/hr.png); }'
      ]
...
`)
  ...
  describe('when iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd:select-all is triggered', function () {
    it('selects the entire Markdown preview', function () {
      const filePath = atom.project
        .getDirectories()[0]
        .resolve('subdir/code-block.md')
      const preview2 = new IansasciidocPreviewfrompackagesmarkdownpreview20240610nomdView({ filePath })
      jasmine.attachToDOM(preview2.element)

      waitsForPromise(() => preview.renderMarkdown())

      runs(function () {
        atom.commands.dispatch(preview.element, 'iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd:select-all')
      ...
      runs(() => {
        atom.commands.dispatch(preview2.element, 'iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd:select-all')
        const selection = window.getSelection()
  ...
  describe('when iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd:zoom-in or iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd:zoom-out are triggered', function () {
    it('increases or decreases the zoom level of the markdown preview element', function () {
      ...
      runs(function () {
        ...
        atom.commands.dispatch(preview.element, 'iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd:zoom-in')
        expect(getComputedStyle(preview.element).zoom).toBeGreaterThan(
          originalZoomLevel
        )
        atom.commands.dispatch(preview.element, 'iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd:zoom-out')
  ...
  describe('when GitHub styles are enabled', () => {
    beforeEach(() => {
      atom.config.set('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.useGitHubStyle', true)
      atom.config.set('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.gitHubStyleMode', 'light')
    })
    it('uses the GitHub styles', () => {
      ...
      atom.config.set('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.gitHubStyleMode', 'dark')
      expect(preview.element.dataset.useGithubStyle).toBe('dark')
      expect(getComputedStyle(paragraph).color).toBe(expectedColors.dark)

      atom.config.set('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.gitHubStyleMode', 'auto')
