...
function chooseRender(text, filePath) {
  ...
  const isAsciidoctor = "true";
  if (isAsciidoctor) { return renderAsciidoctor (text, filePath) } ;

  if (atom.config.get("iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.useOriginalParser")) {
    ...
    let html = atom.ui.markdown.render(text, {
      renderMode: "fragment",
      filePath: filePath,
      breaks: atom.config.get('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.breakOnSingleNewline'),
      useDefaultEmoji: true,
      sanitizeAllowUnknownProtocols: atom.config.get('iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.allowUnsafeProtocols')
    })
...
//       'iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.allowUnsafeProtocols'
//     )
//   })
//
//   const template = document.createElement('template')
...
// needed in nomd "reassignEditorToLanguage not defined"
...
// needed for nomd "highlightCodeBlocks not defined"
...
// needed for nomd "makeAtomEditorNonInteractive not defined"
...
// appears not needed in nomd - I'm not sure about this
//  as it is called in exports.toHTML: 
// await highlightCodeBlocks(div, grammar, null, convertAtomEditorToStandardElement)
