...
function getUserLanguageIds() {
  try {
    let usersLanguageIDs = atom.config.get("iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.customSyntaxHighlightingLanguageIdentifiers");
    ...
    atom.notifications.addError(`Unable to load Iansasciidoc Previewfrompackagesmarkdownpreview20240610nomd Custom Syntax Highlighting Language Identifiers\n${err.toString()}`);
    return {};
  }
}
...
function getLanguageIds() {
  ...
  let preferredLanguageID = atom.config.get("iansasciidoc-previewfrompackagesmarkdownpreview20240610nomd.syntaxHighlightingLanguageIdentifier");
